...
"use strict";function isNumeric(a){return!isNaN(parseFloat(a))&&isFinite(a)}function getValuesArr(a){var b=[];for(key in a)a.hasOwnProperty(key)&&b.push(a[key]);return b}var token=token||$('meta[name="csrf-token"]').attr("content"),TeevyLanding=angular.module("teevyLanding",[]);TeevyLanding.controller("appCtrl",["$log","$http",function(a,b){var c=this;c.modalType=null,c.errors={},c.resetErrors=function(a){void 0===a?c.errors={}:delete c.errors[a]},c.changeModalType=function(a){c.resetErrors(),c.password="",c.modalType=a},c.logIn=function(){b.post("/login",{user_session:{login:c.login,password:c.password},authenticity_token:token}).then(function(){window.location="/app"},function(){c.errors.login="Invalid login or password"})},c.signUp=function(){b.post("/users",{user:{login:c.login,email:c.email,password:c.password,password_confirmation:c.password_confirmation},authenticity_token:token}).then(function(){window.location="/app"},function(a){c.errors.email=a.data.email&&"Email "+a.data.email[0],c.errors.login=a.data.login&&"Username "+a.data.login[0],c.errors.password=a.data.password&&"Password "+a.data.password[0]})}}]),TeevyLanding.directive("modal",["$document",function(a){return{scope:{modal:"="},link:function(b,c){function d(f){e?c.is(f.target)||c.find(f.target).length||(b.modal=null,b.$apply(),a.unbind("click",d)):e=!0}var e=!1;a.bind("click",d)}}}]),TeevyLanding.directive("passwordConfirmation",function(){return function(a,b){function c(){e.setCustomValidity(d.value!=e.value?"Passwords Don't Match":"")}var d=b.find(".password")[0],e=b.find(".confirmation")[0];d.onchange=c,e.onchange=c}});var token=token||$('meta[name="csrf-token"]').attr("content"),TeevyApp=angular.module("teevyApp",["ngRoute","truncate"]);TeevyApp.config(["$routeProvider",function(a){a.when("/dashboard",{templateUrl:"dashboard.html",controller:"dashboardCtrl"}).when("/explore",{templateUrl:"explore.html",controller:"exploreCtrl"}).otherwise({redirectTo:"/dashboard"})}]),TeevyApp.controller("dashboardCtrl",["$scope","Subscription",function(a,b){b.all().then(function(b){a.subscriptions=b})}]),TeevyApp.controller("exploreCtrl",["$scope","Show","Subscription",function(a,b,c){b.all().then(function(b){a.shows=b}),a.subscribe=function(a){a.pending=!0,c.all().then(function(){c.create(a.id).then(function(b){a.pending=!1,a.subscription_id=b.id})})},a.unsubscribe=function(b){b.pending=!0,c.delete(b.subscription_id).then(function(){b.pending=!1,b.subscription_id=null,a.$apply()})}}]),TeevyApp.controller("userCtrl",["$scope","User",function(a,b){b.getUserInfo().then(function(b){a.user=b.data}),a.logout=b.logoutUser,a.showMenu=!1}]),TeevyApp.controller("appCtrl",["$scope","$location","Show","Subscription","filterFilter",function(a,b,c,d,e){c.all().then(function(b){a.shows=b}),a.showSearchResults=!1,a.isActive=function(a){return b.path().slice(1)===a?"active":void 0},a.subscribe=function(a){a.pending=!0,d.all().then(function(){d.create(a.id).then(function(b){a.pending=!1,a.subscription_id=b.id})})},a.unsubscribe=function(b){b.pending=!0,d.delete(b.subscription_id).then(function(){b.pending=!1,b.subscription_id=null,a.$apply()})},a.$watch("search",function(b){a.filteredShows=e(a.shows,{title:b}),a.showSearchResults=b&&b.length>2})}]),TeevyApp.factory("User",["$http","$log",function(a,b){return{getUserInfo:function(){return a.get("/users/current")},logoutUser:function(){a.post("/logout",{authenticity_token:token}).then(function(){window.location="/"},function(){b.error("error during logout...")})}}}]),TeevyApp.factory("Show",["$http","$q","$log",function(a,b){var c=[],d={};return{all:function(){if(0===c.length)return a.get("/shows").then(function(a){return angular.forEach(a.data,function(a,b){d[a.id]=b,c[b]=a}),c});var e=b.defer();return e.resolve(c),e.promise},get:function(a){var e=b.defer();return e.resolve(c[d[a]]),e.promise}}}]),TeevyApp.factory("Episode",["$http","$q","$log",function(a,b){var c={};return{all:function(d){if(c["show_"+d]){var e=b.defer();return e.resolve(c["show_"+d]),e.promise}return a.get("/shows/"+d+"/episodes/").then(function(a){return c["show_"+d]=a.data,c["show_"+d]})}}}]),TeevyApp.factory("Subscription",["$http","$q","$log","Episode",function(a,b,c){var d=[],e={};return{all:function(){if(0===d.length)return a.get("/users/current/subscriptions").then(function(a){return angular.forEach(a.data,function(a,b){e[a.id]=b,d[b]=a}),d});var c=b.defer();return c.resolve(d),c.promise},get:function(a){if(d[e[a]]){var c=b.defer();return c.resolve(d[e[a]]),c.promise}return api.all().then(function(){return d[e[a]]})},update:function(b,f,g){return a.put("/users/current/subscriptions/"+b+".json",{option:f,episode_id:g,authenticity_token:token}).then(function(a){return d[e[b]].next_unseen=a.data.next_unseen,d[e[b]].watched_all=a.data.watched_all,a.data},function(){c.error("Could not set next episode on server")})},create:function(b){return a.post("/users/current/subscriptions",{subscription:{show_id:b},authenticity_token:token}).then(function(a){return d.push(a.data),e[a.data.id]=d.length-1,d[e[a.data.id]]})},"delete":function(a){return $.ajax("/users/current/subscriptions/"+a,{type:"DELETE",data:{authenticity_token:token}}).then(function(b){return d.splice(e[a],1),b},function(a,b,d){c.error("error while deletion: ",a,b,d)})}}}]),TeevyApp.directive("dropdown",["$document",function(a){return{scope:{open:"="},link:function(b,c){function d(a){c.is(a.target)||c.find(a.target).length||(b.open=!1,b.$apply())}var e=c.find(".dropdown");b.$watch("open",function(b){b?(e.show(),a.bind("click",d)):(e.hide(),a.unbind("click",d))})}}}]),TeevyApp.directive("subscriptionTile",["$log","$timeout","Show","Episode","Subscription",function(a,b,c,d,e){return{restrict:"E",templateUrl:"subscription-tile.html",scope:{subscription:"="},link:function(a){a.isAired=function(a){return a&&moment().format()>a.aired_at},a.config=!1,a.manual=!1,a.initialized=!0,a.sawLastAired=!a.isAired(a.subscription.next_unseen),a.subscription.next_unseen||(a.config=!0,a.initialized=!1),a.selectEpisode=function(b){a.isAired(b)&&(a.selected_episode=b)},a.setEpisode=function(c,d){a.pending=!0,e.update(a.subscription.id,c,d).then(function(){a.pending=!1,a.config=!1,console.log(a.subscription.next_unseen),a.sawLastAired=!a.isAired(a.subscription.next_unseen),b(function(){a.initialized=!0},400)})},a.enterConfig=function(){a.manual=!1,a.config=!0},a.exitConfig=function(){a.config=!1},a.enterManual=function(){d.all(a.subscription.show.id).then(function(b){a.episodes=b,a.selected_episode=b[0],a.manual=!0})},a.unsubscribe=function(){c.get(a.subscription.show.id).then(function(b){var c=b;a.pending=!0,e.delete(a.subscription.id).then(function(){a.pending=!1,c.subscription_id=null,a.$apply()})})}}}}]),TeevyApp.filter("date",function(){return function(a,b){return moment(a).isValid()?moment(a).format(b):a}});